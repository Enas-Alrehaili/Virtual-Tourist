...
class CustomImageView: UIImageView {
    ...
    func loadImage(with url: URL) {
        ...
        URLSession.shared.dataTask(with: url) { (data, response, error) in
            ...
            guard let downnloadedImage = UIImage(data: data!) else { return }
            imagesCache.setObject(downnloadedImage, forKey: url.absoluteString as NSString)
             DispatchQueue.main.async {
                self.image = downnloadedImage
                self.photo.set(image: downnloadedImage)
